...
export class N8NDocumentationMCPServer {
  ...
  private validateToolParams(toolName: string, args: any, legacyRequiredParams?: string[]): void {
    try {
      ...
      case 'n8n_audit_instance':
        ...
        break;
      case 'n8n_get_node_config':
        validationResult = args.workflowId && args.nodeName
          ? { valid: true, errors: [] }
          : { valid: false, errors: [
              ...(args.workflowId ? [] : [{ field: 'workflowId', message: 'workflowId is required' }]),
              ...(args.nodeName ? [] : [{ field: 'nodeName', message: 'nodeName is required' }]),
            ]};
        break;
      case 'n8n_read_node_field':
        validationResult = args.workflowId && args.nodeName && args.fieldPath
          ? { valid: true, errors: [] }
          : { valid: false, errors: [
              ...(args.workflowId ? [] : [{ field: 'workflowId', message: 'workflowId is required' }]),
              ...(args.nodeName ? [] : [{ field: 'nodeName', message: 'nodeName is required' }]),
              ...(args.fieldPath ? [] : [{ field: 'fieldPath', message: 'fieldPath is required' }]),
            ]};
        break;
  ...
  async executeTool(name: string, args: any): Promise<any> {
    ...
    switch (name) {
      ...
      case 'n8n_get_node_config':
        this.validateToolParams(name, args, ['workflowId', 'nodeName']);
        return n8nHandlers.handleGetNodeConfig(args, this.instanceContext);

      case 'n8n_read_node_field':
        this.validateToolParams(name, args, ['workflowId', 'nodeName', 'fieldPath']);
        return n8nHandlers.handleReadNodeField(args, this.instanceContext);
