...
export async function handleGetWorkflow(args: unknown, context?: InstanceContext): Promise<McpToolResponse> {
  try {
    ...
    const schema = z.object({
      id: z.string(),
      nodeFilter: z.array(z.string()).optional(),
    });
    const { id, nodeFilter } = schema.parse(args);

    const workflow = await client.getWorkflow(id);
    let result = stripActiveVersion(workflow);

    // Apply nodeFilter if provided
    if (nodeFilter && nodeFilter.length > 0) {
      const filterSet = new Set(nodeFilter);
      const filteredNodes = workflow.nodes.filter(n => filterSet.has(n.name));
      const notFound = nodeFilter.filter(name => !workflow.nodes.some(n => n.name === name));

      // Filter connections to only those involving filtered nodes
      ...
      for (const [sourceName, conns] of Object.entries(workflow.connections)) {
        if (filteredNodeNames.has(sourceName)) {
          // Only keep connections to other filtered nodes
          const filteredConns: any[] = [];
          const connValues = Object.values(conns) as any[];
          for (const conn of connValues) {
            const targetNode = workflow.nodes.find(n => n.id === conn.node);
            if (targetNode && filteredNodeNames.has(targetNode.name)) {
              filteredConns.push(conn);
            }
          }
          if (filteredConns.length > 0) {
            filteredConnections[sourceName] = filteredConns;
          }
        }
      }
      ...
      result = {
        ...
        _filterApplied: {
          ...
          connectionCount: Object.keys(filteredConnections).length,
          requested: nodeFilter.length,
...
/**
 * Get a single node's full configuration from a workflow by node name.
 * Returns parameters, type, position, and metadata without downloading the full workflow.
 */
export async function handleGetNodeConfig(args: unknown, context?: InstanceContext): Promise<McpToolResponse> {
  try {
    const client = ensureApiConfigured(context);
    const schema = z.object({
      workflowId: z.string(),
    ...
    const { workflowId, nodeName, fields } = schema.parse(args);

    const workflow = await client.getWorkflow(workflowId);
    const node = workflow.nodes.find(n => n.name === nodeName);

    if (!node) {
      const availableNodes = workflow.nodes.map(n => n.name);
      return {
        success: false,
        error: `Node "${nodeName}" not found in workflow ${workflowId}. Available nodes: ${availableNodes.join(', ')}`,
      };
    }

    // Filter parameters if fields specified
    let parameters = node.parameters || {};
    if (fields && fields.length > 0) {
      const filteredParams: Record<string, unknown> = {};
      for (const field of fields) {
        // Support dot-separated paths like "parameters.jsCode"
        const parts = field.split('.');
        if (parts[0] === 'parameters' && parts.length > 1) {
          const paramKey = parts.slice(1).join('.');
          if (paramKey in parameters) {
            filteredParams[paramKey] = parameters[paramKey];
          }
        } else if (field in parameters) {
          filteredParams[field] = parameters[field];
        }
      }
...
/**
 * Read the current value of a specific field from a workflow node.
 * Use this to inspect a field before editing it with patchNodeField.
 */
export async function handleReadNodeField(args: unknown, context?: InstanceContext): Promise<McpToolResponse> {
  try {
    const client = ensureApiConfigured(context);
    const schema = z.object({
      workflowId: z.string(),
    ...
    const { workflowId, nodeName, fieldPath } = schema.parse(args);
    ...
    // Resolve dot-separated field path
    ...
    for (const part of parts) {
      if (current === null || current === undefined || typeof current !== 'object') {
        return {
          success: false,
          error: `Cannot traverse "${fieldPath}": "${part}" does not exist on ${typeof current}`,
        };
      }
      if (!(part in current)) {
        return {
          success: false,
          error: `Field "${fieldPath}" not found on node "${nodeName}". Available top-level keys: ${Object.keys(current).join(', ')}`,
        };
      }
    ...
    const length = valueType === 'string' ? (value as string).length : undefined;
